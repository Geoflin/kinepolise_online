...
foreach ($pdo_kinepolise->query('SELECT DateSeanceBegin FROM `kinepolise_cinema2_seance_cinema1` WHERE DateSeanceBegin LIKE "'.$fusion.'" AND SalleName LIKE "'.$SalleName.'" ', PDO::FETCH_ASSOC) as $search) {
...
foreach ($pdo_kinepolise->query('SELECT SalleName FROM `kinepolise_cinema2_seance_cinema1` WHERE `DateSeanceBegin` >= "'.$fusion.'" AND `DateSeanceBegin` <= "'.$DateFinSeance.'" AND `SalleName` = "'.$SalleName.'" ', PDO::FETCH_ASSOC) as $creneau) {
...
foreach ($pdo_kinepolise->query('SELECT * FROM `kinepolise_cinema2_seance_cinema1`', PDO::FETCH_ASSOC) as $seance) {
  $Id_creation= $seance['Id'];
                ...
                //on ajoute date de fin séance
                $sql = "UPDATE `kinepolise_cinema2_seance_cinema1` SET `DateSeanceEnd` = '".$DateFinSeance."' WHERE `seance_cinema1`.`Id` = '".$seance['Id']."' ";
                $count = $pdo_kinepolise->exec($sql);
                $pdo_kinepolise = null;
              };
            } else {
